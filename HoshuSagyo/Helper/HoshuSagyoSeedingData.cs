    ...
    public static class HoshuSagyoSeedingData
    {
        ...
        public static void SeedMasterTable(this ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<OnseiOtoModel>().HasData(
                new OnseiOtoModel { OtoShubetsu = 1, OtoShubetsuNaiyo = "共通", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoModel { OtoShubetsu = 2, OtoShubetsuNaiyo = "施設", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoModel { OtoShubetsu = 3, OtoShubetsuNaiyo = "電力", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoModel { OtoShubetsu = 4, OtoShubetsuNaiyo = "通信", RegisterDateTime = now, UpdateDateTime = now }
                );

            // 音声応答詳細
            modelBuilder.Entity<OnseiOtoShosaiModel>().HasData(
                new OnseiOtoShosaiModel { Id = 1, OtoShubetsu = 1, Message = "関係者以外の人はいませんか", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoShosaiModel { Id = 2, OtoShubetsu = 1, Message = "作業場所は間違っていませんか", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoShosaiModel { Id = 3, OtoShubetsu = 1, Message = "作業員は全員揃いましたか", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoShosaiModel { Id = 4, OtoShubetsu = 1, Message = "道具の忘れ物はありませんか", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoShosaiModel { Id = 5, OtoShubetsu = 2, Message = "施設の責任者から作業許可は出ていますか", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoShosaiModel { Id = 6, OtoShubetsu = 2, Message = "各エリアの施錠は行いましたか", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoShosaiModel { Id = 7, OtoShubetsu = 3, Message = "電気の供給が停止していますか", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoShosaiModel { Id = 8, OtoShubetsu = 3, Message = "電気の供給を再開しましたか", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoShosaiModel { Id = 9, OtoShubetsu = 4, Message = "通信状態をメンテナンス状態にしましたか", RegisterDateTime = now, UpdateDateTime = now },
                new OnseiOtoShosaiModel { Id = 10, OtoShubetsu = 4, Message = "通信状態を通常状態に戻しましたか", RegisterDateTime = now, UpdateDateTime = now }
                );
